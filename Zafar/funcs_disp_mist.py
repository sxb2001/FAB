...
def train_model_disp_mist(x, y, x_control, loss_function, EPS, cons_params=None):
    ...
    if loss_function == "logreg":
        ...
        loss = cvxpy.sum(
            logistic(cvx.multiply(-y, x @ w))) / num_points  # we are converting y to a diagonal matrix for consistent
    ...
    if cons_params is not None:
        ...
        elif not cons_params["take_initial_sol"]:
